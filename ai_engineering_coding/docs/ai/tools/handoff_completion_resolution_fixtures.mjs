...
const cases = [
  ...
  ["review_current_no_completion", base({ handoff_observation: null }), "NOT_COMPLETE", "HANDOFF_NOT_COMPLETE"],
...
let passed = 0;
for (const [id, input, status, reason, refs] of cases) { let ok = true; try { assertResult(input, status, reason, refs); } catch { ok = false; } console.log(`${ok ? "PASS" : "FAIL"} ${id}`); if (ok) passed++; }
...
for (const [id, input] of contradictions) { const result = resolveHandoffCompletion(input); const ok = result.resolution_status === "invalid_input"; console.log(`${ok ? "PASS" : "FAIL"} ${id}`); if (ok) passed++; }

const malformed = [
  ["null_root", null], ["array_root", []], ["empty_root", {}], ["unknown_root_key", { ...base(), extra: true }], ["null_implementation", base({ implementation: null })], ["invalid_implementation", base({ implementation: { status: "MAYBE" } })], ["invalid_verification", base({ verification: { status: "PASSED" } })], ["invalid_requirement", base({ review_requirement: { status: "MAYBE" } })], ["invalid_review_result", base({ review_result: { status: "DONE" } })], ["invalid_reassessment", base({ scope_reassessment: { required: "no" } })], ["null_observation_allowed", base({ handoff_observation: null })], ["array_observation", base({ handoff_observation: [] })], ["unknown_observation_field", base({ handoff_observation: { ...completed(), extra: true } })], ["missing_identity", base({ handoff_observation: { evidence_refs: ["handoff.evidence"] } })], ["malformed_identity", base({ handoff_observation: completed({ identity: {} }) })], ["missing_evidence", base({ handoff_observation: completed({ evidence_refs: [] }) })], ["bad_evidence_ref", base({ handoff_observation: completed({ evidence_refs: ["https://example.com"] }) })], ["oversized_fingerprint", base({ handoff_observation: completed({ identity: identity({ implementation_fingerprint: "x".repeat(129) }) }) })], ["raw_handoff_prose", base({ handoff_observation: { ...completed(), summary: "done" } })], ["raw_diff", base({ handoff_observation: { ...completed(), diff: "source" } })],
];
for (const [id, input] of malformed) { let result; try { result = resolveHandoffCompletion(input); } catch { result = null; } const ok = result?.resolution_status === "invalid_input"; console.log(`${ok ? "PASS" : "FAIL"} ${id}`); if (ok) passed++; }
...
const deterministic = JSON.stringify(resolveHandoffCompletion(orderedA)) === JSON.stringify(resolveHandoffCompletion(orderedB)); console.log(`${deterministic ? "PASS" : "FAIL"} unordered_full_output_equality`); if (deterministic) passed++;
const frozen = structuredClone(orderedB); const before = JSON.stringify(frozen); const deepFreeze = (value) => { if (value && typeof value === "object" && !Object.isFrozen(value)) { Object.freeze(value); Object.values(value).forEach(deepFreeze); } }; deepFreeze(frozen); const immutable = resolveHandoffCompletion(frozen).status === "COMPLETE" && JSON.stringify(frozen) === before; console.log(`${immutable ? "PASS" : "FAIL"} frozen_input_not_mutated`); if (immutable) passed++;
...
const workflowOk = workflowReady.status === "READY" && workflowReady.stage === "HANDOFF" && workflowComplete.status === "COMPLETE" && workflowComplete.stage === "NONE"; console.log(`${workflowOk ? "PASS" : "FAIL"} workflow_compatibility`); if (workflowOk) passed++;
console.log(`Handoff completion fixture suite: ${passed} passed`); process.exitCode = passed === cases.length + staleDimensions.length + contradictions.length + malformed.length + 4 ? 0 : 1;
